...
def run(dispatch_unit_information, dispatch_unit_capacity_bids, initial_conditions, regulated_interconnectors,
        ...
        market_cap_and_floor, enable_interconector_segment_order_constraints=True):
    ...
    var_definitions, inter_variable_bounds, region_req_by_row = \
        create_lp_as_dataframes(dispatch_unit_information, dispatch_unit_capacity_bids, initial_conditions,
                                ...
                                enable_interconector_segment_order_constraints=True)
...
def create_lp_as_dataframes(gen_info_raw, capacity_bids_raw, unit_solution_raw, inter_direct_raw,
                            ...
                            mnsp_capacity_bids, enable_interconector_segment_order_constraints=True):
    t1 = time()
    ...
    # Create constraints that ensure interconnector segments are dispatched in the correct order.
    if enable_interconector_segment_order_constraints:
        inter_seg_dispatch_order_constraints = \
            interconnectors.create_inter_seg_dispatch_order_constraints(inter_bounds, max_con_index, max_var_index)
        max_con_index = hf.max_constraint_index(inter_seg_dispatch_order_constraints)
        optional_constraint_sets.append(inter_seg_dispatch_order_constraints)
    ...
    coefficient_data_list = [bidding_constraints,
                             ...
                             joint_capacity_constraints] + optional_constraint_sets
...
def create_inequality_types(constraint_sets):
    # For each constraint row determine the inequality type and save in a list of ascending order according to row
    # index.
    processed_sets = []
    for constraint_set in constraint_sets:
        constraint_set = constraint_set.drop_duplicates(subset=['ROWINDEX']).copy()
        constraint_set = constraint_set.loc[:, ('ROWINDEX', 'CONSTRAINTTYPE', 'RHSCONSTANT')].copy()
        processed_sets.append(constraint_set)
    combined_type_data = pd.concat(processed_sets)
